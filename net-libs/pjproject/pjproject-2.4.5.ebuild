...
src_prepare() {
        epatch "${FILESDIR}/ring_endianness.patch" \
                "${FILESDIR}/ring_gnutls.patch" \
                "${FILESDIR}/ring_ice_config.patch" \
                "${FILESDIR}/ring_ipv6.patch" \
                "${FILESDIR}/ring_multiple_listeners.patch" \
                "${FILESDIR}/ring_notestsapps.patch" \
                "${FILESDIR}/ring_pj_ice_sess.patch" \
...
src_install() {
	...
	# hack to remove portaudio
	rm "${D}/usr/lib64/libportaudio-x86_64-pc-linux-gnu.a"
	rm "${D}/usr/lib64/libportaudio.so"
	rm "${D}/usr/lib64/libportaudio.so.2"
}
